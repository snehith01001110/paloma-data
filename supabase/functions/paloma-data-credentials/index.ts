...
const ALLOWED_WORKFLOWS = new Map<string, Set<string>>([
  [
    "snehith01001110/paloma-data/.github/workflows/sync.yml@refs/heads/main",
    new Set(["push", "schedule", "workflow_dispatch"]),
  ],
  [
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const allowedEvents = ALLOWED_WORKFLOWS.get(String(payload.workflow_ref));
    if (
      ...
      !allowedEvents?.has(String(payload.event_name))
    ) {
      return unauthorized();
    }
