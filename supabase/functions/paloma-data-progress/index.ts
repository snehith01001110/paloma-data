...
Deno.serve(async (req: Request) => {
  ...
  try {
    const [latestRuns, stagedRows, linkedRows, reviewRows, overallRows] = await Promise.all([
      sql`
      ...
      sql`
        select source,
          ...
          count(distinct establishment_id)::bigint as linked_establishment_count
      ...
      sql`
        select source,
          ...
          count(distinct source_record_id) filter (where state = 'pending')::bigint as pending_review_record_count
      ...
      const [countRows, items] = await Promise.all([
        ...
        sql`
          ...
          select e.id::text, e.name, e.address, e.city, e.region, e.status,
            e.data_quality_score, e.last_verified_at, pt.slug as primary_type_slug,
            p.sources, p.source_count
          from public.establishments e
          join provenance p on p.establishment_id = e.id
          ...
          order by e.last_verified_at desc nulls last, e.name
      ...
      const [countRows, items] = await Promise.all([
        ...
        sql`
          select rq.id::text, rq.source, rq.reason, rq.confidence, rq.created_at,
            rq.candidate_establishment_id::text,
            sr.name, sr.address, sr.city, sr.region, sr.primary_type_slug,
