...
class FieldResolver:
    ...
    def _identity_confidence(self, conn, establishment_id: str) -> float:
        rows = conn.execute(
            """
            select source, max(match_confidence)::float as confidence
    ...
    def _resolve_display_name(self, conn, establishment_id: str) -> list[dict[str, Any]]:
        ...
        for normalized_value, rows in grouped.items():
            ...
            independent_sources = {str(row["source"]) for row in rows if row["source"] != "canonical_seed"}
            agreement_bonus = min(0.08, 0.04 * max(0, len(independent_sources) - 1))
    ...
    def _type_confidence(self, conn, establishment_id: str, current_type: str, identity: float) -> float:
        ...
        sources = {str(row["source"]) for row in matching}
        return min(0.995, best + min(0.06, 0.03 * max(0, len(sources) - 1)))
