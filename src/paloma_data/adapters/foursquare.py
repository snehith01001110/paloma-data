...
_FSQ_REQUIRED_FIELDS = frozenset(
    {
        "fsq_place_id",
        "name",
        "latitude",
...
class FoursquareAdapter:
    ...
    def _scan(self) -> Iterator[SourceRecord]:
        ...
        catalog_options: dict[str, str] = {
            ...
            # These are part of the PyIceberg connection snippet generated by the
            # Foursquare Places Portal. Keep the production adapter aligned with the
            # credential probe instead of relying on client-library defaults.
        ...
        missing = sorted(_FSQ_REQUIRED_FIELDS - available)
        if missing:
            raise RuntimeError(
                "FSQ OS schema is missing required fields: " + ", ".join(missing)
            )
        # Keep the open-source adapter strictly inside the published Apache-2.0 schema. Rich
        # fields are obtained only through the separately licensed verification adapter.
        selected_fields = tuple(field for field in _FSQ_FIELDS if field in available)
        row_filter = And(
    ...
    def _to_record(self, row: dict[str, Any]) -> SourceRecord | None:
        ...
        refreshed_at = _datetime_or_none(row.get("date_refreshed"))
        stale = refreshed_at is None or refreshed_at < datetime.now(timezone.utc) - timedelta(
            days=FSQ_RECENCY_DAYS
        )
        if stale:
            quality_flags = tuple(sorted({*quality_flags, "stale"}))
        hard_closed = bool(
            {"closed", "delete", "doesnt_exist", "does_not_exist"} & set(quality_flags)
        )
        status = "closed" if closed_at or hard_closed else "unknown" if stale else "open"
        ...
        if is_consumer_facing_type(classification.primary_type_slug) and not consumer_facing:
            quality_flags = tuple(sorted({*quality_flags, "consumer_identity_conflict"}))
        return SourceRecord(
            source=self.source,
            ...
            # Only derive from fields in the published OS projection. Rich attributes that may
            # appear in fixtures/future schemas belong to the separately licensed adapter.
            setting_slugs=_objective_settings({}, category_labels),
...
def _consumer_identity_is_supported(
    name: str,
